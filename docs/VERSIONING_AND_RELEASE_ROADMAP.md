...
## `0.9.200.6` — Combat 2.0 Packet B5: Brasshaven / Redstone Combat-Training Proof
...
Game State stays 18 because B5 adds no durable state family. Technique instruction reuses capability progression and semantic events; the partial-ammo validator repair makes the existing Game State 18 contract accept the stackable ammo shape introduced by B4. Data 67 records Varric/POI combat-training service metadata. No supported-save migration is added.
...
## `0.9.300.7` — Advanced Combat Packet 7: Radiant Arc Propagation Foundation
...
Data 74 changes the existing Radiant Arc definition from generic adept single-target damage into explicit Light magical resolution plus synchronous target-to-target `arc` geometry. The primary target is recipient 1; each later recipient is the nearest living unhit opponent within two derived formation units of the previous recipient. Encounter order breaks distance ties, the action caps at three total recipients, and no damage falloff is introduced in this foundation packet.

`combatGeometryEngine` adds deterministic arc propagation evidence: primary target, jump range, target cap, repeat-target policy, ordering law, and per-recipient jump/origin/distance/position. The existing geometric target-expansion path in `abilityEngine` applies Radiant Arc independently to all selected recipients, and existing explicit per-recipient attention records hostility only for landed effects.

No future propagation deadline or state survives the completed action. Game State therefore remains 21; no save migration, timer/task owner, movement state, LOS, or pathfinding state is added.

Behavioral/data implementation freeze `65f10a96d4e479b758981f3798efbfc1ddf059ec` passed Check #2106 / run `33569913910` with **889/889 tests**, Repository Audit, Census, Benchmark 3, and Benchmark Sample. Pages #2236 / run `33569912530` passed. Validation-only PR #405 also passed Check #2107 and was closed without merge after evidence collection.
...
This revision converts the remaining compact starter caster/offhand cluster while preserving singular equipment identity and existing loadout semantics.
...
A3 adds:
- `starterCasterOffhandProductionCatalog` with four process definitions;
- canonical production paths for existing Ash Staff, Maple Wand, Iron Buckler, and Brass Ring IDs;
- `pack-starter-caster-offhand-equipment` owning four existing item refs plus four recipe refs;
- A1 Field Knife `cutting` bindings for both wood-equipment recipes;
- real two-handed/offhand, weapon-cadence, shield/stat, ring/accessory, and current-schema save/load proof.

The recipes reuse established Elderwood/Silvermaple/common-component/brass and Redstone tempered-iron/rivet supply chains. Data advances because four process definitions and one shared Pack-v2 ownership tranche are new authored canonical content. Canonical item count remains unchanged because all outputs are existing equipment IDs.

Validated implementation freeze `d672f3ab90ec46c6ca9ef4beb85cef1fbfe5353d` passed Check #2240 / run `33671247638` with **916/916 tests**, Repository Audit, Census, Benchmark 3, and Benchmark Sample.

Census result:
- canonical items 410 unchanged;
- recipes/processes 247;
- regional/shared packs 42;
- pack-owned records 1,351.
