...
const SPELL_SCHOOLS = Object.freeze({
    'school-embercraft': school({ id: 'school-embercraft', name: 'Embercraft', tradition: 'A practical study of heat, ignition, and tightly contained elemental force.', tags: ['elemental', 'heat', 'projection'] }),
...
const ABILITIES = Object.freeze({
    'ability-ember-dart': ability({ id: 'ability-ember-dart', name: 'Ember Dart', kind: 'spell', schoolId: 'school-embercraft', capabilityId: 'spell-ember-dart', tags: ['magic', 'offensive', 'heat'], contexts: ['combat'], target: { kind: 'enemy' }, activation: { durationSeconds: 6, interruptible: true }, cooldownSeconds: 12, costs: { mp: 10 }, effects: [{ type: 'damage', recipient: 'target', stat: 'int', base: 8, coefficient: 1.4 }] }),
    ...
    'ability-ridge-breaker': ability({
        id: 'ability-ridge-breaker', name: 'Ridge Breaker', kind: 'technique', capabilityId: 'technique-ridge-breaker', tags: ['martial', 'weapon-technique', 'axe', 'redstone'], contexts: ['combat'], target: { kind: 'enemy' }, activation: { durationSeconds: 0, interruptible: false }, cooldownSeconds: 10, costs: { tp: 300 },
        effects: [{ type: 'damage', recipient: 'target', stat: 'str', base: 7, coefficient: 1.15 }],
    }),
    'ability-rivet-guard': ability({
        id: 'ability-rivet-guard', name: 'Rivet Guard', kind: 'technique', capabilityId: 'technique-rivet-guard', tags: ['martial', 'weapon-technique', 'sword', 'defensive', 'redstone'], contexts: ['combat'], target: { kind: 'enemy' }, activation: { durationSeconds: 0, interruptible: false }, cooldownSeconds: 10, costs: { tp: 250 },
        effects: [{ type: 'damage', recipient: 'target', stat: 'str', base: 4, coefficient: 0.85 }, { type: 'status', recipient: 'self', status: { id: 'status-rivet-guard', name: 'Rivet Guard', category: 'buff', durationSeconds: 15, stackGroup: 'rivet-guard', stackRule: 'replace', modifiers: { defense: 3 }, flags: { braced: true } } }],
    }),
    'ability-forge-spark': ability({ id: 'ability-forge-spark', name: 'Forge Spark', kind: 'spell', schoolId: 'school-embercraft', capabilityId: 'spell-forge-spark', tags: ['magic', 'offensive', 'heat', 'redstone'], contexts: ['combat'], target: { kind: 'enemy' }, activation: { durationSeconds: 5, interruptible: true }, cooldownSeconds: 14, costs: { mp: 12 }, effects: [{ type: 'damage', recipient: 'target', stat: 'int', base: 10, coefficient: 1.5 }] }),
    'ability-ironbound-ward': ability({
        id: 'ability-ironbound-ward', name: 'Ironbound Ward', kind: 'spell', schoolId: 'school-ward-lore', capabilityId: 'spell-ironbound-ward', tags: ['magic', 'warding', 'defensive', 'redstone'], contexts: ['combat', 'exploration'], target: { kind: 'self' }, activation: { durationSeconds: 5, interruptible: true }, cooldownSeconds: 24, costs: { mp: 8 },
        effects: [{ type: 'status', recipient: 'self', status: { id: 'status-ironbound-ward', name: 'Ironbound Ward', category: 'buff', durationSeconds: 36, stackGroup: 'ward-defense', stackRule: 'replace', modifiers: { defense: 6 }, flags: { magicalWard: true, ironbound: true } } }],
    }),
    ...
    'ability-barkskin-ward': ability({
        id: 'ability-barkskin-ward', name: 'Barkskin Ward', kind: 'spell', schoolId: 'school-ward-lore', capabilityId: 'spell-barkskin-ward', tags: ['magic', 'warding', 'defensive', 'elderwood'], contexts: ['combat', 'exploration'], target: { kind: 'self' }, activation: { durationSeconds: 5, interruptible: true }, cooldownSeconds: 22, costs: { mp: 8 },
...
function ability(definition) { return deepFreeze({ id: String(definition.id), name: String(definition.name), kind: definition.kind, schoolId: definition.schoolId ? String(definition.schoolId) : null, capabilityId: String(definition.capabilityId), tags: [...(definition.tags ?? [])], contexts: [...(definition.contexts ?? [])], target: { ...(definition.target ?? {}) }, activation: { durationSeconds: Math.max(0, Math.floor(Number(definition.activation?.durationSeconds) || 0)), interruptible: definition.activation?.interruptible === true }, cooldownSeconds: Math.max(0, Math.floor(Number(definition.cooldownSeconds) || 0)), costs: { ...(definition.costs ?? {}) }, effects: (definition.effects ?? []).map((effect) => ({ ...effect, status: effect.status ? { ...effect.status, modifiers: { ...(effect.status.modifiers ?? {}) }, flags: { ...(effect.status.flags ?? {}) } } : undefined })) }); }
