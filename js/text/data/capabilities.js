...
const CAPABILITY_DEFINITIONS = Object.freeze({
    ...
    'technique-guarded-cut': capability({
        ...
        use: {
            contexts: ['combat'],
            requiredSkills: [{ skillId: 'sword', min: 1 }],
            mainHandTags: ['sword'],
            resources: { tp: 250 },
        },
    }),
    'technique-shadow-feint': capability({
        ...
        learning: {
            anyDiscipline: [
                { disciplineId: 'shadowhand', minLevel: 1 },
                { disciplineId: 'veilrunner', minLevel: 2 },
            ],
        },
        use: {
            contexts: ['combat'],
            requiredSkills: [{ skillId: 'dagger', min: 1 }],
            mainHandTags: ['dagger'],
            resources: { tp: 200 },
        },
    }),
    'practical-field-dressing': capability({
        id: 'practical-field-dressing',
        name: 'Field Dressing',
        type: 'practical',
        tags: ['resource-recovery', 'anatomy', 'fieldcraft'],
        learning: { open: true },
        use: {
            contexts: ['resourceRecovery'],
            requiredToolTags: ['cutting'],
        },
    }),
    'practical-ore-survey': capability({
        id: 'practical-ore-survey',
        name: 'Ore Survey',
        type: 'practical',
        tags: ['gathering', 'mining', 'fieldcraft'],
        learning: { open: true },
        use: {
            contexts: ['gathering'],
...
export function getCapability(capabilityId) {
    return CAPABILITY_DEFINITIONS[String(capabilityId ?? '').trim()] ?? null;
}
...
export function validateCapabilityCatalog() {
    ...
    for (const definition of listCapabilities()) {
        ...
        if (learning.open !== true && !(learning.anyDiscipline?.length)) {
            issues.push(`${definition.id} requires an open or discipline learning path.`);
        }
        ...
        for (const context of use.contexts ?? []) {
            if (!CAPABILITY_CONTEXTS.includes(context)) issues.push(`${definition.id} references unknown context ${context}.`);
        }
        ...
        for (const field of ['mainHandTags', 'requiredToolTags', 'requiredPreparationTags', 'requiredFlags']) {
            if (!Array.isArray(use[field])) issues.push(`${definition.id} use.${field} must be an array.`);
        }
...
function capability(definition) {
    return deepFreeze({
        ...
        learning: {
            open: definition.learning?.open === true,
            anyDiscipline: (definition.learning?.anyDiscipline ?? []).map((entry) => ({
                disciplineId: String(entry.disciplineId),
                minLevel: Math.max(1, Math.floor(Number(entry.minLevel) || 1)),
            })),
        },
        use: {
            contexts: [...(definition.use?.contexts ?? [])],
            requiredSkills: (definition.use?.requiredSkills ?? []).map((entry) => ({
                skillId: String(entry.skillId),
                min: Math.max(1, Math.floor(Number(entry.min) || 1)),
            })),
            mainHandTags: [...(definition.use?.mainHandTags ?? [])],
            requiredToolTags: [...(definition.use?.requiredToolTags ?? [])],
            requiredPreparationTags: [...(definition.use?.requiredPreparationTags ?? [])],
            requiredFlags: [...(definition.use?.requiredFlags ?? [])],
            resources: { ...(definition.use?.resources ?? {}) },
        },
    });
}
