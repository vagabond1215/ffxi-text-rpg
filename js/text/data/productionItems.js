...
export const PRODUCTION_ITEM_CATALOG_VERSION = 12;
...
export function getProductionItem(itemId) { const key = String(itemId ?? '').trim(); const definition = PRODUCTION_ITEM_DEFINITIONS[key] ?? null; return definition ? normalizeItem(definition) : getIngredientLuxuryProductionItem(key) ?? getGreatMereProductionItem(key) ?? getIronspineProductionItem(key) ?? getHeadwaterProductionItem(key) ?? getMaterialFoundationProductionItem(key); }
export function listProductionItems() { return [...Object.values(PRODUCTION_ITEM_DEFINITIONS).map((definition) => normalizeItem(definition)), ...listIngredientLuxuryProductionItems(), ...listGreatMereProductionItems(), ...listIronspineProductionItems(), ...listHeadwaterProductionItems(), ...listMaterialFoundationProductionItems()]; }
