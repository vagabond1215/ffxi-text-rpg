...
export function createMenuActionList(session, modal = null) {
    if (modal === 'login') {
        return [
            ...(session?.accounts ?? []).map((account) => uiAction(`account:${account.id}`, account.displayName, 'account.select', { accountId: account.id })),
    ...
    return [
        ...characters.map((character) => uiAction(`character:${character.id}`, `${character.name} - ${character.job} Lv.${character.level}`, 'character.select', { characterId: character.id })),
        commandAction('newCharacter', characters.length ? 'New Character' : 'Create Character', '/newcharacter'),
