...
export function resolveBasicAttack(battle, attackerId, defenderId, options = {}) {
    ...
    const attackProfile = options.attackProfile ?? {};
    const resolution = resolveCombatDamage(attacker, defender, {
        stat: attackProfile.stat ?? 'str',
        base: Math.max(1, Number(attacker.combat?.level) || 1),
        coefficient: Number.isFinite(Number(attackProfile.coefficient)) ? Number(attackProfile.coefficient) : 0.5,
        resolution: {
            delivery: 'melee',
            channel: attackProfile.channel ?? 'physical',
            damageType: attackProfile.damageType ?? 'physical',
            element: attackProfile.element ?? null,
            elementSource: attackProfile.element ? (attackProfile.elementSource ?? 'weaponKata') : null,
            accuracyModel: 'physical',
            accuracyModifier: Number(attackProfile.accuracyModifier) || 0,
            resistanceModel: attackProfile.resistanceModel ?? 'physicalDefense',
            defensePenetration: Number(attackProfile.defensePenetration) || 0,
            criticalEligible: false,
        },
    }, { rng: options.rng ?? battle.rng ?? Math.random });

    if (!resolution.hit) {
        appendBattleLog(battle, options.actionName ? `${attacker.identity.name} uses ${options.actionName} but misses ${defender.identity.name}.` : `${attacker.identity.name} misses ${defender.identity.name}.`);
        return {
            ok: true,
            outcome: 'miss',
            attackerId,
    ...
    appendBattleLog(battle, options.actionName ? `${attacker.identity.name} uses ${options.actionName} on ${defender.identity.name} for ${damage} damage.` : `${attacker.identity.name} hits ${defender.identity.name} for ${damage} damage.`);
...
export function resolveRangedAttack(battle, attackerId, defenderId, options = {}) {
    const attacker = getCombatant(battle, attackerId);
    const defender = getCombatant(battle, defenderId);
    if (!attacker || !defender || attacker.battle?.defeated || defender.battle?.defeated || getCombatantSide(attacker) === getCombatantSide(defender)) {
        return { ok: false, outcome: 'unavailable', attackerId, defenderId, hit: false, damage: 0, defeatedTarget: Boolean(defender?.battle?.defeated), resolution: null };
    }
    ...
    if (!resolution.hit) {
        appendBattleLog(battle, `${attacker.identity.name}'s ranged attack misses ${defender.identity.name}.`);
        return { ok: true, outcome: 'miss', attackerId, defenderId, hit: false, damage: 0, hitChance: resolution.accuracy.chance, roll: resolution.accuracy.roll, defeatedTarget: false, resolution };
    }
    ...
    attacker.resources.tp = Math.min(attacker.combat.resources.maxTp, attacker.resources.tp + 100);
    defender.resources.tp = Math.min(defender.combat.resources.maxTp, defender.resources.tp + 30);
    appendBattleLog(battle, `${attacker.identity.name} strikes ${defender.identity.name} at range for ${damage} damage.`);

    const defeatedTarget = defender.resources.hp <= 0;
    if (defeatedTarget) {
        defender.battle.defeated = true;
        appendBattleLog(battle, `${defender.identity.name} is defeated.`);
    }
    updateBattlePhase(battle);
    return { ok: true, outcome: defeatedTarget ? 'defeated-target' : 'hit', attackerId, defenderId, hit: true, damage, hpBefore, hpAfter: defender.resources.hp, hitChance: resolution.accuracy.chance, roll: resolution.accuracy.roll, defeatedTarget, resolution };
}
