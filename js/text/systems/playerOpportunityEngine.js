...
export function createPlayerOpportunityModel(state) {
    ...
    if (!experience.guide.met) {
        const entry = freezeOpportunity({
            ...
            progress: `Learn how ${origin.nationName} connects to ${origin.regionalHorizon} and where a newcomer can begin making a useful name.`,
...
function createPreparationOpportunity({ state, origin, starterItem, starterItemId, starterEquipped, starterCarried }) {
    ...
    let nextAction = null;
    let summary = `${starterItem.name} is equipped and ready for the field.`;
    ...
    return freezeOpportunity({
        ...
        progress: 'With the right tool equipped, more field work and recovery choices become practical.',
        status,
        requirements,
        action: nextAction,
    });
...
function createLivelihoodOpportunity({ state, origin, destination, source, starterItem, starterEquipped, inDestination }) {
    ...
    return freezeOpportunity({
        ...
        summary: `Working ${source.name} can produce useful regional material while improving your ${humanize(source.proficiencyId)} practice.`,
        ...
        progress: `More ${humanize(source.proficiencyId)} practice, useful material, and familiarity with ${origin.regionalHorizon}.`,
        status,
        requirements: [
            ...
            ...source.requiredToolTags.map((tag) => requirement(`Tool capability: ${humanize(tag)}`, starterEquipped && (starterItem?.tags ?? []).includes(tag))),
        ],
        blockers,
        action: nextAction,
    });
...
function createRegionalLoopLivelihood({ state, origin, destination, source, starterItem, starterEquipped, inDestination }) {
    ...
    if (outputQuantity > 0) {
        return freezeOpportunity({
            ...
            summary: `You brought Redstone ore back to ${returnPlace?.name ?? origin.nationName} and smelted it into a copper ingot at the forge.`,
    ...
    if (activeWork) {
        ...
        return freezeOpportunity({
            ...
            summary: isProduction
                ? 'The ore is already in the work. Finish smelting to collect the ingot.'
                : `You are still working at ${source.name}. Finish the task to collect the ore and add to your ${humanize(source.proficiencyId)} practice.`,
            ...
            progress: isProduction ? loop.largerAmbition : `${humanize(source.proficiencyId)} practice and enough ore to justify the return trip.`,
            status: OPPORTUNITY_STATUSES.ACTIVE,
    ...
    if (!starterEquipped && resourceQuantity < loop.targetResourceQuantity) {
        return freezeOpportunity({
            ...
            summary: `${source.name} can supply Redstone copper, but you need your field tool equipped before mining can begin.`,
            reason: 'Preparation constrains practical work; the Journal does not bypass equipment requirements.',
            progress: `${humanize(source.proficiencyId)} practice and Redstone copper once the tool is ready.`,
    ...
    if (resourceQuantity < loop.targetResourceQuantity) {
        ...
        if (!inDestination) {
            ...
            return freezeOpportunity({
                ...
                progress: `${humanize(source.proficiencyId)} practice first; metalworking practice after you bring the ore home.`,
                status: step.status,
                requirements: [
                    requirement(`Equip ${starterItem?.name ?? 'the required field tool'}`, starterEquipped),
                    ...step.requirements,
        ...
        return freezeOpportunity({
            ...
            summary: `Spend the time to mine the remaining ore at ${source.name}; your tool, practice, and the condition of the seam determine whether you can begin.`,
            reason: 'Bringing enough ore home creates a reason for the return trip and a concrete processing step.',
            progress: `${humanize(source.proficiencyId)} practice now; metalworking practice after the ore reaches a forge.`,
    ...
    if (state.currentPlaceId !== loop.returnPlaceId) {
        ...
        return freezeOpportunity({
            ...
            summary: `You carry ${resourceQuantity} Redstone copper ore. The next useful step is to bring it back to a settlement forge.`,
            reason: 'Returning matters because settlements convert field gains into processing, trade, equipment, and larger plans.',
            progress: `Your ${humanize(source.proficiencyId)} practice is already yours; bringing the ore home opens metalworking progress.`,
            status: step.status,
            requirements: [
    ...
    if (productionCheck.ok) {
        return freezeOpportunity({
            ...
            summary: 'You have the ore and access to a forge. Start smelting; the ingot will be ready when the work is finished.',
    ...
    if (stationMissing && workstation?.placeId === state.currentPlaceId) {
        return freezeOpportunity({
            ...
            summary: `${workstation.name} has the forge you need for ${production?.name ?? 'copper processing'}.`,
    ...
    return freezeOpportunity({
        ...
        summary: 'You have the ore back in town, but the current workshop or preparation still blocks the next step.',
...
function createTrainingOpportunity({ state, origin, destination, inDestination }) {
    ...
    return freezeOpportunity({
        ...
        summary: `${destination.name} has lower-risk field threats where you can practice combat without committing to a harder expedition.`,
        ...
        progress: 'Combat practice, discipline experience, and familiarity with field danger.',
        status,
...
function createExplorationOpportunity({ state, origin, destination, destinationVisited, inDestination }) {
    ...
    return freezeOpportunity({
        ...
        summary: `Travel beyond the safe wards into ${origin.regionalHorizon} and learn the ground by going there.`,
        ...
        progress: `A known route between ${origin.nationName} and ${destination.name}, plus whatever places and landmarks you discover along the way.`,
        status,
...
function freezeModel(origin, entries, recommendedOpportunityId) {
    return Object.freeze({
        ...
        prompt: 'Choose one useful next step. You can change course whenever another lead matters more.',
