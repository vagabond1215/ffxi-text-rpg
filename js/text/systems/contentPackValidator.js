...
function validateCommitmentQuestRecord(pack, commitment, context, issues, label) {
    ...
    if (commitment.prerequisiteCommitmentIds !== undefined && !Array.isArray(commitment.prerequisiteCommitmentIds)) issues.push(`${label}.prerequisiteCommitmentIds must be an array.`);
...
function validateCompanionRecord(pack, record, context, issues, label) {
    ...
    if (record.recruitment?.requiredCommitmentIds !== undefined && !Array.isArray(record.recruitment.requiredCommitmentIds)) issues.push(`${label}.recruitment.requiredCommitmentIds must be an array.`);
