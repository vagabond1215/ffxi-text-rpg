...
test('version manifest separates product package persistence data and focused cleanup versions', () => {
    assert.equal(PRODUCT_VERSION, '0.8.600.41');
    ...
    assert.equal(VERSION.codename, 'Canonical Player Statuses');
    ...
    assert.match(describeVersion(), /Product: 0\.8\.600\.41/);
    ...
    assert.match(describeVersion(), /Codename: Canonical Player Statuses/);
    assert.match(describeVersion(), /Compatibility: pre-release-current-schema/);
    assert.match(describeSystemVersions(), /validation: 0\.33\.0/);
