...
test('version manifest separates product package persistence data and focused cleanup versions', () => {
    assert.equal(PRODUCT_VERSION, '0.8.600.40');
    ...
    assert.equal(VERSION.codename, 'Strict Player Equipment');
    ...
    assert.match(describeVersion(), /Product: 0\.8\.600\.40/);
    ...
    assert.match(describeVersion(), /Codename: Strict Player Equipment/);
    assert.match(describeVersion(), /Compatibility: pre-release-current-schema/);
    assert.match(describeSystemVersions(), /validation: 0\.32\.0/);
