...
test('version manifest separates product package persistence data and focused cleanup versions', () => {
    assert.equal(PRODUCT_VERSION, '0.8.600.50');
    ...
    assert.equal(VERSION.codename, 'Derived NPC World Projection');
    ...
    assert.match(describeVersion(), /Product: 0\.8\.600\.50/);
    assert.match(describeVersion(), /Package: 0\.8\.600/);
    assert.match(describeVersion(), /Account Save: 5/);
    assert.match(describeVersion(), /Game State: 10/);
    assert.match(describeVersion(), /Data: 37/);
    assert.match(describeVersion(), /Benchmark: 3/);
    assert.match(describeVersion(), /Codename: Derived NPC World Projection/);
    assert.match(describeVersion(), /Compatibility: pre-release-current-schema/);
    assert.match(describeSystemVersions(), /validation: 0\.41\.0/);
    assert.match(describeSystemVersions(), /npcWorldProjection: 0\.1\.0/);
    ...
    assert.match(describeSystemVersions(), /saveEncoding: 0\.7\.0/);
