...
test('version manifest separates product package persistence data and scale-content versions', () => {
    ...
    assert.equal(VERSION.codename, 'Existing Field-Tool Conversion Proof');
    ...
    assert.match(describeVersion(), /Product: 0\.9\.400\.2/);
    ...
    assert.match(describeVersion(), /Data: 76/);
    assert.match(describeVersion(), /Benchmark: 3/);
    assert.match(describeVersion(), /Codename: Existing Field-Tool Conversion Proof/);
    ...
    assert.match(describeSystemVersions(), /regionalContentPacks: 0\.23\.0/);
