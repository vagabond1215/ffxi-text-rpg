...
test('version manifest separates product package persistence data and focused cleanup versions', () => {
    assert.equal(PRODUCT_VERSION, '0.8.600.28');
    ...
    assert.equal(VERSION.codename, 'Strict World Simulation');
    ...
    assert.match(describeVersion(), /Product: 0\.8\.600\.28/);
    ...
    assert.match(describeVersion(), /Codename: Strict World Simulation/);
    ...
    assert.match(describeSystemVersions(), /validation: 0\.20\.0/);
