...
test('version manifest separates product package persistence data and focused cleanup versions', () => {
    assert.equal(PRODUCT_VERSION, '0.8.600.43');
    ...
    assert.equal(VERSION.codename, 'Player Persistence Integration');
    ...
    assert.match(describeVersion(), /Product: 0\.8\.600\.43/);
    ...
    assert.match(describeVersion(), /Codename: Player Persistence Integration/);
