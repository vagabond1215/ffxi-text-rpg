...
test('version manifest separates product package persistence data and focused cleanup versions', () => {
    assert.equal(PRODUCT_VERSION, '0.8.600.26');
    ...
    assert.equal(VERSION.codename, 'Strict Ecology Registry');
    ...
    assert.match(describeVersion(), /Product: 0\.8\.600\.26/);
    ...
    assert.match(describeVersion(), /Codename: Strict Ecology Registry/);
    ...
    assert.match(describeSystemVersions(), /validation: 0\.18\.0/);
