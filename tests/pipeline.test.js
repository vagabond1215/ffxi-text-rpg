...
test('version manifest separates product package persistence data and focused cleanup versions', () => {
    assert.equal(PRODUCT_VERSION, '0.8.600.15');
    ...
    assert.equal(VERSION.codename, 'Transport Task Release');
    ...
    assert.match(describeVersion(), /Product: 0\.8\.600\.15/);
    ...
    assert.match(describeVersion(), /Codename: Transport Task Release/);
    ...
    assert.match(describeSystemVersions(), /lifecycleHarness: 0\.6\.0/);
