...
test('version manifest separates product package persistence data and focused cleanup versions', () => {
    assert.equal(PRODUCT_VERSION, '0.8.600.25');
    ...
    assert.equal(VERSION.codename, 'Strict Resource Opportunities');
    ...
    assert.match(describeVersion(), /Product: 0\.8\.600\.25/);
    ...
    assert.match(describeVersion(), /Codename: Strict Resource Opportunities/);
    ...
    assert.match(describeSystemVersions(), /validation: 0\.17\.0/);
