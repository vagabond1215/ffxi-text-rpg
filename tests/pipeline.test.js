...
test('version manifest separates product package persistence data and focused cleanup versions', () => {
    assert.equal(PRODUCT_VERSION, '0.8.600.27');
    ...
    assert.equal(VERSION.codename, 'Strict Character Runtime');
    ...
    assert.match(describeVersion(), /Product: 0\.8\.600\.27/);
    ...
    assert.match(describeVersion(), /Codename: Strict Character Runtime/);
    ...
    assert.match(describeSystemVersions(), /validation: 0\.19\.0/);
