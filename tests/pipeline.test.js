...
test('version manifest separates product package persistence data and focused cleanup versions', () => {
    assert.equal(PRODUCT_VERSION, '0.8.600.29');
    ...
    assert.equal(VERSION.codename, 'Strict Player Capabilities');
    ...
    assert.match(describeVersion(), /Product: 0\.8\.600\.29/);
    ...
    assert.match(describeVersion(), /Codename: Strict Player Capabilities/);
    ...
    assert.match(describeSystemVersions(), /validation: 0\.21\.0/);
