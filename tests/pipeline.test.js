...
test('version manifest separates product package persistence data and focused cleanup versions', () => {
    assert.equal(PRODUCT_VERSION, '0.8.600.24');
    ...
    assert.equal(VERSION.codename, 'Strict Continuity Registries');
    ...
    assert.match(describeVersion(), /Product: 0\.8\.600\.24/);
    ...
    assert.match(describeVersion(), /Codename: Strict Continuity Registries/);
    ...
    assert.match(describeSystemVersions(), /validation: 0\.16\.0/);
