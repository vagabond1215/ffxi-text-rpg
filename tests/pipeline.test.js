...
test('version manifest separates product package persistence data and focused cleanup versions', () => {
    assert.equal(PRODUCT_VERSION, '0.8.600.30');
    ...
    assert.equal(VERSION.codename, 'Strict Inventory State');
    ...
    assert.match(describeVersion(), /Product: 0\.8\.600\.30/);
    ...
    assert.match(describeVersion(), /Codename: Strict Inventory State/);
    ...
    assert.match(describeSystemVersions(), /validation: 0\.22\.0/);
