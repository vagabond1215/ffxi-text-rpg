...
test('version manifest separates product package persistence data and focused cleanup versions', () => {
    assert.equal(PRODUCT_VERSION, '0.8.600.16');
    ...
    assert.equal(VERSION.codename, 'Ability Resource Task Release');
    ...
    assert.match(describeVersion(), /Product: 0\.8\.600\.16/);
    ...
    assert.match(describeVersion(), /Codename: Ability Resource Task Release/);
    ...
    assert.match(describeSystemVersions(), /lifecycleHarness: 0\.7\.0/);
