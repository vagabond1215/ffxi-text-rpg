...
test('version manifest separates product package persistence data and focused cleanup versions', () => {
    assert.equal(PRODUCT_VERSION, '0.8.600.49');
    ...
    assert.equal(VERSION.codename, 'Strict Active Battle Player Link');
    ...
    assert.match(describeVersion(), /Product: 0\.8\.600\.49/);
    ...
    assert.match(describeVersion(), /Codename: Strict Active Battle Player Link/);
    assert.match(describeVersion(), /Compatibility: pre-release-current-schema/);
    assert.match(describeSystemVersions(), /validation: 0\.40\.0/);
