...
test('content pack index records expanded stable ownership without changing canonical ids', () => {
    ...
    console.log('HEADWATER_HIGHLAND_MEASURED_PACK_INDEX', JSON.stringify({ packIds: index.packIds, recordCounts: index.recordCounts, ownerCount: index.ownerCount }));
    ...
    assert.equal(index.recordCounts.items, 375);
    assert.equal(index.recordCounts.recipes, 227);
