...
test('combined production catalog consumes at least three quarters of current canonical raw resources', () => {
    ...
    console.log('ELDERWOOD_REPAIR_MEASURED_RAW_UTILIZATION', JSON.stringify({ raw: rawIds.size, used: usedRawIds.size }));
    assert.equal(rawIds.size, 135);
    assert.equal(usedRawIds.size, 124);
